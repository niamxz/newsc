...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/niamxz/regip/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/niamxz/regip/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/niamxz/regip/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/niamxz/regip/main/ip | grep $MYIP | awk '{print $3}')
fi
...
# Status ExpiRED Active | JOEXMASTER
Info="(${c2}Active${NC})"
Error="(${red}Expired${NC})"
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(curl https://raw.githubusercontent.com/niamxz/regip/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
export sem=$( curl -s https://raw.githubusercontent.com/niamxz/newsc/main/version)
...
clear
echo -e " ${z}╭══════════════════════════════════════════════════════════╮${NC}"
echo -e " ${z}│$NC\033[41m         JOEXMASTER SCRIPTER          $NC${z}│$NC"
echo -e " ${z}╰══════════════════════════════════════════════════════════╯${NC}"
echo -e " ${z}╭══════════════════════════════════════════════════════════╮${NC}"
